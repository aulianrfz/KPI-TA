...
<div class="container py-4">
    ...
    <div class="tab-content" id="invoiceTabsContent">
        <div class="tab-pane fade show active" id="invoice" role="tabpanel">
            <div id="invoice-to-print" class="card card-body p-4 p-md-5" style="background-color: #EAF2FF; color: #333;">
                <div class="d-flex justify-content-between align-items-start mb-4">
                    <div>
                        <h2 class="fw-bolder" style="color: #000;">INVOICE</h2>
                        <p class="text-muted mb-0">#{{ $peserta->pendaftar->id ?? '' }}</p>
                    </div>
                    <div class="text-end">
                ...
                <div class="row">
                    <div class="col-lg-8">
                        ...
                        <div class="mb-4">
                            <p class="mb-1"><span class="fw-bold" style="width: 150px; display: inline-block;">Nama Institusi</span>: {{ $peserta->institusi }}</p>
                            <p class="mb-1"><span class="fw-bold" style="width: 150px; display: inline-block;">Cabang Lomba</span>: {{ $peserta->pendaftar?->mataLomba?->nama_lomba ?? '-' }}</p>
                            @if ($peserta->tim->isNotEmpty())
                                <p class="mb-1"><span class="fw-bold" style="width: 150px; display: inline-block;">Nama Tim</span>: {{ $peserta->tim->first()->nama_tim }}</p>
                            @endif
                        </div>
                        <div>
                            <h5 class="fw-bold mb-3">Team Details</h5>
                    ...
                    <div class="col-lg-4 mt-4 mt-lg-0">
                        <div class="card shadow-sm border-0 text-dark">
                            <div class="card-header bg-white pb-0"><h5 class="fw-bolder">Bill details</h5></div>
                            <div class="card-body">
                                @php
                                ...
                                <div class="d-flex justify-content-between"><span class="text-muted">Jumlah Peserta</span><span>{{ $jumlahPeserta }}</span></div>
                            </div>
                            <div class="card-footer bg-white border-0 pt-0">
                                <hr class="my-2">
                                <div class="d-flex justify-content-between fw-bolder fs-5"><span>Total</span><span>Rp{{ number_format($total, 0, ',', '.') }}</span></div>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="text-end mt-4 no-print">
                    <button class="btn btn-danger" onclick="window.print()"><i class="bi bi-file-earmark-pdf-fill"></i> Download PDF</button>
                </div>
            </div>
        </div>
                            ...
                            <div class="card-body">
                                ...
                                <div class="mb-3">
                                    ...
                                    <div class="input-group">
                                        ...
                                        <button class="btn btn-sm btn-outline-secondary ms-3" type="button" onclick="copyToClipboard('[Isi Nomor Rekening]', this)">
                                            <i class="bi bi-clipboard"></i> Salin
                                        </button>
                                    </div>
                                </div>
                                <hr>
                                <p class="mb-0 text-muted" style="font-size: 0.9em;">Pastikan nama dan nomor rekening sudah benar sebelum melakukan transfer.</p>
                            </div>
                        </div>
                    </div>
                    <div class="col-lg-7">
                        ...
                        <ol class="list-unstyled">
                            <li class="d-flex mb-4">
                                ...
                                <div><strong class="d-block">Unggah Bukti Pembayaran</strong>Setelah transfer berhasil, buka tab "Bayar" untuk mengunggah bukti pembayaran Anda.</div>
                            </li>
                            <li class="d-flex mb-4">
                                <i class="bi bi-patch-check-fill fs-3 text-success me-3"></i>
                                <div><strong class="d-block">Tunggu Verifikasi</strong>Tim kami akan memverifikasi pembayaran Anda dalam waktu 1-2 hari kerja.</div>
                            </li>
                            <li class="d-flex mb-4">
                                <i class="bi bi-envelope-check-fill fs-3 text-info me-3"></i>
                                <div><strong class="d-block">Pembayaran Diterima</strong>Jika valid, status pembayaran akan berubah dan QR Code untuk acara akan dikirim melalui email.</div>
                            </li>
                             <li class="d-flex">
                                <i class="bi bi-headset fs-3 text-muted me-3"></i>
                                <div><strong class="d-block">Butuh Bantuan?</strong>Jika mengalami kendala, jangan ragu untuk menghubungi panitia melalui kontak yang tersedia.</div>
                            </li>
                        </ol>
                    </div>
                </div>
                ...
                <form action="{{ route('pembayaran.upload', $peserta->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-3">
                        ...
                        <div class="mb-3 text-center">
                            <label for="bukti_pembayaran" class="form-label d-block"><i class="bi bi-cloud-upload" style="font-size: 48px; color: #007bff;"></i></label>
