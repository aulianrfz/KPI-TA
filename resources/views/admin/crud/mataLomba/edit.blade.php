        ...
        <form action="{{ route('mataLomba.update', $mataLomba->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-3">
                ...
                <input type="text" name="nama_lomba" id="nama_lomba" class="form-control"
                    value="{{ $mataLomba->nama_lomba }}" required>
            </div>
            ...
            <div class="mb-3">
                ...
                <input type="number" name="min_peserta" id="min_peserta" class="form-control"
                    value="{{ $mataLomba->min_peserta }}" required>
            </div>

            <div class="mb-3">
                <label for="maks_peserta" class="form-label">Maksimal Peserta</label>
                <input type="number" name="maks_peserta" id="maks_peserta" class="form-control"
                    value="{{ $mataLomba->maks_peserta }}" required>
            </div>

            <div class="mb-3">
                <label for="maks_total_peserta" class="form-label">Maksimal Total Peserta</label>
                <input type="number" name="maks_total_peserta" id="maks_total_peserta" class="form-control"
                    value="{{ $mataLomba->maks_total_peserta }}" required>
            </div>
            ...
            <div class="mb-3">
                ...
                <input type="number" name="biaya_pendaftaran" id="biaya_pendaftaran" class="form-control"
                    value="{{ $mataLomba->biaya_pendaftaran }}" required>
            </div>

            <div class="mb-3">
                <label for="durasi" class="form-label">Durasi Perlombaan (menit)</label>
                <input type="number" name="durasi" id="durasi" class="form-control" value="{{ $mataLomba->durasi }}"
                    required>
            </div>
            ...
            <div class="mb-3">
                ...
                <select name="jenis_pelaksanaan" id="jenis_pelaksanaan" class="form-select" required>
                    ...
                    <option value="Offline" {{ $mataLomba->jenis_pelaksanaan == 'Offline' ? 'selected' : '' }}>Offline
                    </option>
                </select>
            </div>
            ...
            <div class="mb-3">
                ...
                <textarea name="deskripsi" id="deskripsi" class="form-control" rows="4"
                    required>{{ $mataLomba->deskripsi }}</textarea>
            </div>
            ...
            <div class="mb-3">
                ...
                @if ($mataLomba->foto_kompetisi)
                    <p class="mt-2">Foto lama: <img src="{{ asset('storage/' . $mataLomba->foto_kompetisi) }}" alt="Foto"
                            width="50"></p>
                @endif
            </div>
