...
@section('content')
    <style>
        ...
        .fc .fc-toolbar-chunk {
            display: flex;
            align-items: center;
            /* KUNCI UTAMA: Sejajarkan semua item di dalamnya secara vertikal */
        }
        ...
        .fc .fc-next-button {
            ...
            color: #555 !important;
            border-radius: 50%;
            /* Opsi: buat tombol jadi lingkaran agar lebih rapi */
        ...
        .fc-icon-chevron-right::before {
            ...
            /* Gunakan font yang karakternya lebih seimbang */
            font-size: 1.5rem;
            /* Sesuaikan ukuran icon */
            font-weight: bold;
            line-height: 1;
            /* Pastikan icon pas di tengah */
        }
        ...
        .fc .fc-day-other {
            ...
            background-image: repeating-linear-gradient(45deg, #f9f9f9, #f9f9f9 5px, #ffffff 5px, #ffffff 10px);
        }
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body row">
                        ...
                        <div class="form-group col-md-6">
                            ...
                            <input type="text" id="waktuMulai" class="form-control" required placeholder="HH:mm"
                                maxlength="5" autocomplete="off">
                            <small id="errorMulai" class="text-danger"></small>
                        </div>
                        <div class="form-group col-md-6">
                            <label for="waktuSelesai">Waktu Selesai</label>
                            <input type="text" id="waktuSelesai" class="form-control" required placeholder="HH:mm"
                                maxlength="5" autocomplete="off">
                            <small id="errorSelesai" class="text-danger"></small>
                        </div>
                    </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            document.getElementById('timeForm').addEventListener('submit', function (e) {
                ...
                // Reset error display dulu
                document.getElementById('errorMulai').textContent = '';
                document.getElementById('errorSelesai').textContent = '';

                let hasError = false;

                if (!waktuMulai || !isValidTimeFormat(waktuMulai)) {
                    document.getElementById('errorMulai').textContent = 'Format waktu salah. Gunakan format HH:mm';
                    hasError = true;
                }
                if (!waktuSelesai || !isValidTimeFormat(waktuSelesai)) {
                    document.getElementById('errorSelesai').textContent = 'Format waktu salah. Gunakan format HH:mm';
                    hasError = true;
                }
            ...
            function restrictTimeInput(input) {
                ...
                input.addEventListener('blur', function () {
                    ...
                    // Jangan munculin alert atau fokus ulang
                    const errorId = this.id === 'waktuMulai' ? 'errorMulai' : 'errorSelesai';
                    document.getElementById(errorId).textContent = isValidTimeFormat(this.value)
                        ? ''
                        : 'Format waktu salah. Gunakan format HH:mm';
                });
            }
    ...
    @if(session('notEnoughTime'))
        <script>
            window.onload = () => {
                const modal = new bootstrap.Modal(document.getElementById('notEnoughTimeModal'));
                modal.show();
            };
        </script>
    @endif

    <div class="modal fade" id="notEnoughTimeModal" tabindex="-1">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">Waktu Tidak Cukup</h5>
                </div>
                <div class="modal-body">Durasi waktu yang tersedia tidak cukup untuk menjadwalkan semua lomba.</div>
