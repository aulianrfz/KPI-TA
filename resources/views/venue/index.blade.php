...
<style>
    ...
    .table-header-dark-custom {
        background-color: #343a40 !important; /* Warna abu-abu gelap (Bootstrap thead-dark) */
    }
    ...
    .table tbody td {
        text-align: center;
        vertical-align: middle;
        font-size: 0.9rem; /* Ukuran font konten tabel */
    }
    ...
    <div class="table-card">
        <div class="card-header">
            <form action="{{ route('venue.index') }}" method="GET" class="d-flex">
                <div class="input-group input-group-sm input-group-table-search" style="width: 300px;">
                    <input type="text" name="search" class="form-control" placeholder="Cari nama venue..." value="{{ request('search') }}">
                ...
                <tbody>
                    @forelse($venues as $index => $venue)
                        <tr>
                            ...
                            <td>{{ $venue->name }}</td>
                            <td>{{ $venue->alamat ?? '-' }}</td> {{-- Asumsi ada field 'alamat', jika tidak ganti atau hapus --}}
                            <td>{{ $venue->kapasitas ?? '-' }}</td> {{-- Asumsi ada field 'kapasitas', jika tidak ganti atau hapus --}}
                            <td>{{ $venue->jadwal_tersedia ?? '-' }}</td> {{-- Asumsi ada field 'jadwal_tersedia', jika tidak ganti atau hapus --}}
                            <td>
                                <a href="{{ route('venue.edit', $venue->id) }}" class="btn btn-action-edit-custom me-1" title="Edit">
                                ...
                                <button type="button" class="btn btn-action-delete-custom" title="Hapus"
                                        ...
                                        data-action="{{ route('venue.destroy', $venue->id) }}"
        ...
        <div class="card-footer-custom">
            <div class="pagination-info">
                Menampilkan {{ $venues->firstItem() }} sampai {{ $venues->lastItem() }} dari {{ $venues->total() }} entri
            </div>
            <div>
